...
<template>
    ...
    >
        <div class="px-2 flex items-center min-h-[60px] flex-wrap mb-4 py-4">
            <div class="mr-2">{{ trans.selected }}:</div>
            <TransitionGroup name="fade">
                <div
                    v-for="(tag, index) in Array.from(showList)"
                    :key="index"
                    class="flex center m-1 ml-0 p-2 rounded-xl items-center"
                    :style="{ ...buttonStyles }"
                >
                    {{ optionMap[tag] }}
                    <IconButton
        ...
        <ButtonComponent
            btn-class-name="p-2 bg-gray-300 max-w-fit h-fit m-2 rounded-xl border-none"
            :disabled="showList.size === 0"
            @click="clear"
            text="clear"
    ...
    <div
        class="flex flex-wrap max-w-[50vh] overflow-auto mb-2 flex-col px-2 mt-2"
        id="scroll-container"
    >
